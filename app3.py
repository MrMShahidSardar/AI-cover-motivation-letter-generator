...
st.subheader("Create Motivation Letters & Convert to Cover Letters")
...
def main():
    ...
    with st.expander("Step 3: Upload CV"):
        cv_file = st.file_uploader("Upload CV (Word/Text):", type=["docx", "txt"])
        cv_text = ""
        if cv_file:
    ...
    if st.session_state.generated_letter:
        show_letter("Motivation Letter", st.session_state.generated_letter, "motivation_letter.txt")
        
        if st.button("Convert to Cover Letter"):
            convert_letter_type()

    if st.session_state.cover_letter:
        show_letter("Cover Letter", st.session_state.cover_letter, "cover_letter.txt")
...
def get_system_message(letter_type):
    return (
        "Expert academic writer" if letter_type == "motivation" 
        else "Professional career consultant"
    )

def show_letter(title, content, filename):
    st.subheader(title)
    st.write(content)
    st.download_button(
